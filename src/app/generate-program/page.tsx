        ...
        <div className='flex flex-col min-h-screen text-foreground overflow-hidden pb-6 pt-24'>
            <div className='container mx-auto px-4 h-full max-w-5xl'>
                    ...
                    <Card className='bg-card/90 backdrop-blur-sm border border-border overflow-hidden relative'>
                        <div className='aspect-video flex flex-col items-center justify-center p-6 relative'>
                            <div className={`absolute inset-0 ${isSpeaking ? 'opacity-30' : 'opacity-0'} transition-opacity duration-300`}>
                               <div className='absolute left-0 right-0 top-0.5 -translate-y-0.5 flex justify-center items-center h-20'>
                                  {
                                    [...Array(5)].map((_, i) => (
                                        <div key={i} className={`mx-1 h-16 w-1 bg-primary rounded-full ${isSpeaking ? "animate-sound-wave": ""}`}
                                          style={{
                                            animationDelay: `${i * 0.1}s`,
                                            height: isSpeaking ? `${20 + (i * 15)}%` : "5%",
                                          }} />
                                    ))
                                  }
                               </div>
                            ...
                            <h2 className='text-xl font-bold text-foreground'>RepTracker</h2>
                    ...
                    <Card className={`bg-card/90 backdrop-blur-sm border overflow-hidden relative`}>
                        <div className='aspect-video flex flex-col items-center justify-center p-6 relative'>
                            ...
                            <p className='text-sm text-muted-foreground mt-1'>
                               { user?.firstName ? (user.firstName + " " + (user.lastName || "")).trim() : "Loading..."}
                            </p>
                                 ...
                                 messages.map((msg, index) => (
                                    <div key={index} className='message-item animate-fadeIn'>
                                        <div className="font-semibold text-xs text-muted-foreground mb-1">
                                            { msg.role === 'assistant' ? "RepTracker" : "You"}
                                        </div>
                ...
                <div className='w-full flex justify-center gap-4'>
                    <Button className={`w-40 text-xl rounded-3xl ${ callActive? 'bg-destructive hover:bg-destructive/90' : callEnded ? 'bg-green-600 hover:bg-green-700' : 'bg-primary hover:bg-primary/90'} text-white relative`}
                     onClick={toggleCall}
